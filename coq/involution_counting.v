(*
  ...
  Note (FR) : principe combinatoire abstrait derriere le Theoreme 2
  (Section 5 de l'article) : sous une involution f sur un ensemble fini
  (f(f(x)) = x pour tout x), le nombre d'orbites {x, f(x)} vaut au moins
  la moitie de la taille de l'ensemble. Generalise a tout type a egalite
  decidable, puis instancie concretement pour le cas m=5, k=5 blocs.
  Compile sans aucun axiome (« Closed under the global context ») : une
  preuve purement constructive.
